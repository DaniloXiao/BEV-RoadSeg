...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):#single-gpu
    if len(gpu_ids) > 0:
...
# def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):#multi-gpu
#     if len(gpu_ids) > 0:
#         assert(torch.cuda.is_available())
#         net.to(gpu_ids[0])
...
def define_RoadSeg(num_labels, init_type='xavier', init_gain=0.02, gpu_ids=[]):
...
class RoadSeg(nn.Module):
    ...
    def __init__(self, num_labels):
        ...
        if self.num_resnet_layers == 18:
            ...
            resnet_raw_model = torchvision.models.resnet34(pretrained=True)
            filters = [64, 64, 128, 256, 512]
        elif self.num_resnet_layers == 50:
            resnet_raw_model = torchvision.models.resnet50(pretrained=True)
            filters = [64, 256, 512, 1024, 2048]
        elif self.num_resnet_layers == 101:
            resnet_raw_model = torchvision.models.resnet101(pretrained=True)
            filters = [64, 256, 512, 1024, 2048]
        elif self.num_resnet_layers == 152:
            resnet_raw_model= torchvision.models.resnet152(pretrained=True)
            filters = [64, 256, 512, 1024, 2048]
        else:
