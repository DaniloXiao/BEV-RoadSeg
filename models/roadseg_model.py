...
class RoadSegModel(BaseModel):
    ...
    def initialize(self, opt, dataset):
        ...
        self.visual_names = ['rgb_image', 'label', 'output']
        # specify the models you want to save to the disk. The program will call base_model.save_networks and base_model.load_networks
        self.model_names = ['RoadSeg']

        # load/define networks
        self.netRoadSeg = networks.define_RoadSeg(dataset.num_labels, init_type=opt.init_type, init_gain= opt.init_gain, gpu_ids= self.gpu_ids)
    ...
    def forward(self):
        self.output = self.netRoadSeg(self.rgb_image)#xbp
